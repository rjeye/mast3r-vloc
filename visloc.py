...
from mast3r_src.utils.coarse_to_fine import select_pairs_of_crops, crop_slice
from mast3r_src.utils.collate import cat_collate, cat_collate_fn_map
from mast3r_src.utils.misc import mkdir_for
from mast3r_src.datasets.utils.cropping import crop_to_homography

import mast3r_src.utils.path_to_dust3r  # noqa
from dust3r_src.dust3r.inference import inference, loss_of_one_batch
from dust3r_src.dust3r.utils.geometry import (
    geotrf,
    colmap_to_opencv_intrinsics,
    opencv_to_colmap_intrinsics,
)
from dust3r_src.dust3r.datasets.utils.transforms import ImgNorm
from dust3r_src.dust3r_visloc.datasets import *
from dust3r_src.dust3r_visloc.localization import run_pnp
from dust3r_src.dust3r_visloc.evaluation import (
    get_pose_error,
    aggregate_stats,
    export_results,
)
from dust3r_src.dust3r_visloc.datasets.utils import get_HW_resolution, rescale_points3d
...
def crops_inference(pairs, model, device, batch_size=48, verbose=True):
    ...
    if B < batch_size:
        return loss_of_one_batch(
            pairs, model, None, device=device, symmetrize_batch=False
        )
    preds = []
    for ii in range(0, B, batch_size):
        ...
        preds.append(
            loss_of_one_batch(
                temp_data, model, None, device=device, symmetrize_batch=False
            )
        )  # sequential forward
    # Merge all preds
...
if __name__ == "__main__":
    ...
    for idx in tqdm(range(len(dataset))):
        ...
        query_rgb_tensor, query_K, query_to_orig_max, query_to_resize_max, (HQ, WQ) = (
            resize_image_to_max(
        ...
        query_resolution = get_HW_resolution(
            HQ, WQ, maxdim=maxdim, patchsize=model.patch_embed.patch_size
        )
            ...
            if cache_file is not None and os.path.isfile(cache_file):
                ...
                if args.coarse_to_fine and (maxdim < max(WQ, HQ)):
                    ...
                    if viz_matches > 0:
                        ...
                        for i in range(n_viz):
                            (x0, y0), (x1, y1) = (
                                viz_matches_im_query[i].T,
                                viz_matches_im_map[i].T,
                            )
                            pl.plot(
                                [x0, x1 + W0],
                                [y0, y1],
                    ...
                    map_resolution = get_HW_resolution(
                        HM, WM, maxdim=maxdim, patchsize=model.patch_embed.patch_size
                    )
                    ...
                    for crop_q, crop_b, pair_tag in select_pairs_of_crops(
                        ...
                        c2, _, _, trf2 = crop(
                            query_rgb_tensor, None, None, crop_b, query_K
                        )
                        crops1.append(c1)
                        crops2.append(c2)
                        crops_v1.append(v1)
                    ...
                    if len(crops1) == 0 or len(crops2) == 0:
                        ...
                        to_orig1, to_orig2 = torch.stack(to_orig1), torch.stack(
                            to_orig2
                        )
                        map_crop_view = dict(
                        ...
                        matches_im_map = geotrf(
                            map_to_orig_max, matches_im_map, norm=True
                        )
                else:
                ...
                if cache_file is not None:
                    ...
                    np.savez(
                        ...
                        matches_im_query=matches_im_query,
                        matches_im_map=matches_im_map,
                        matches_conf=matches_conf,
                    )
